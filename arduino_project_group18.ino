...
void loop() {
  ...
  fsm previousState = fsm.UNCOUPLED;
  ...
  while (1) {
    ...
    bool risingEdge = (lastInputState == LOW && currentInputState == HIGH);
    bool fallingEdge = (lastInputState == HIGH && currentInputState == LOW);

    switch (currentState) {
      case fsm.UNCOUPLED:
        if (risingEdge || fallingEdge) {
          if (fallingEdge) {
            tOn = time - lastRisingTime;
            validTOn = (tOn >= tOnMin && tOn <= tOnMax);
          }
          if (risingEdge) {
            period = time - lastRisingTime;
            validPeriod = (period >= periodMin && period <= periodMax);
            if (validPeriod && validTOn) {
              previousState = currentState;
              currentState = fsm.COUPLING;
            }
            lastRisingTime = time;
          }
        }
      ...
      case fsm.COUPLING:
        ...
        if (risingEdge || fallingEdge) {
          if (fallingEdge) {
            ...
            if (!validTOn) {
              previousState = currentState;
              currentState = fsm.UNCOUPLED;
            }
          }
          if (risingEdge) {
            period = time - lastRisingEdge;
            validPeriod = (period >= periodMin && period <= periodMax);
            if (validPeriod) {
              previousState = currentState;
              currentState = fsm.COUPLED;
          ...
          if (!((currentInputState == HIGH && tOn <= tOnMax) || (currentInputState == LOW && period <= periodMax))) {
            validTOn = false;
            previousState = currentState;
            currentState = fsm.UNCOUPLED;
          }
